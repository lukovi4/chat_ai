...
- вместо `startReply` — `admitReply(replyId, request, snapshot)`, вызываемый
  **ровно один раз** на logical reply:
  ...
  - `snapshot` — **committed / persistence-ready** `Conversation` (user Message
    + этот пустой ассистент), зафиксированный до обработки событий reply;
- backend приложения обязан **одной серверной транзакцией** сохранить этот
  snapshot **в точности как передан** и создать (или безопасно присоединиться
  к) единственный Job этого `replyId`; provider нельзя запускать до commit;
- единственное намеренное отличие `snapshot` от локального значения: anchor
  user Message этого reply в нём уже `sent` — статус, который он получит после
  commit'а admission, поэтому восстановление из серверного snapshot не даёт
  ложного незавершённого user-сообщения. Остальные поля и остальные Message
  скопированы без изменений, а собственного неявного `sending → sent` backend
  не делает. **Локально** тот же Message остаётся `sending` до `accepted`, а
  подтверждённый отказ до `accepted` по-прежнему переводит его в `failed`;
- `cancel()`/`dispose()`, выигравший гонку **до фактической отправки
  admission**, отменяет её: Core перепроверяет reply после pre-dispatch await,
  поэтому `admitReply` не вызывается, Job не создаётся и удалённо отменять
  нечего;
